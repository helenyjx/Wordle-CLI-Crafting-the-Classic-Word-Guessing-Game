...
impl Game {
    ...
    fn new() -> Game {
        ...
        println!("Listen! The word chosen for this Game is {}", random_word);
    ...
    fn game_loop(&mut self) {
        ...
        while current_attempt <= self.max_attempts + 1 {
            ...
            if user_word.len() != self.word_length {
                println!("Word is not the right length (⑉꒦ິ^꒦ິ⑉)!");
                continue;
            }
            ...
            match self.words.get(&user_word) {
                Some(_) =>  {
                    ...
                    if temp_attempt.is_win {
                        println!("{}", "Congratulations! You win this game! ( ˶º̬˶ )୨⚑".on_bright_green().black());
                        break;
                    }
